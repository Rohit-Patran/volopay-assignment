...
export default function BlockedCards({data})
{
    ...
    return(
        <section className='flex flex-col justify-center flex-wrap gap-x-6'>
            ...
            <div className="flex flex-row flex-wrap gap-x-6 justify-center">
                {data.filter((item) => {
                    return ((search === "" && val === "")?item.status.toLowerCase() === "blocked" : item.name.toLowerCase().includes(search) && item.card_type === val);
                }).map((item , idx) => (
                    <div className='card my-4 w-2/5 rounded-md px-4 py-2 flex flex-col items-center bg-red-300'>
                        <p className="font-bold uppercase">{item.status}</p>
